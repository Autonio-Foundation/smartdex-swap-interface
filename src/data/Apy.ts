...
export function useApy(dailyRewards: Fraction, totalLiquidity: CurrencyAmount | undefined): number {
    const [currentPrice, setCurrentPrice] = useState(0)

    useEffect(() => {
        fetch('https://api.coingecko.com/api/v3/coins/markets?vs_currency=usd&ids=autonio')
          .then(res => res.json())
          .then(
            (result) => {
